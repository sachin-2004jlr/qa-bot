...
if "rag_engine" not in st.session_state:
    with st.spinner("Initializing..."):
        st.session_state.rag_engine = AdvancedRAG()
    ...
    if st.button("Process Documents"):
        if uploaded_files:
            with st.spinner("Processing..."):
                ...
                status = st.session_state.rag_engine.process_documents(files_path, db_path)
                
                # --- UPDATED: Strictly Text Only (No Icon) ---
...
if prompt := st.chat_input("Ask about your documents..."):
    ...
    if st.session_state.get("db_ready"):
        with st.spinner("Reasoning..."):
            response = st.session_state.rag_engine.query(prompt, db_path)
